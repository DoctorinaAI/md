...
/// Syntax grammar for `clojure`.
///
/// Import this library only when you need `clojure` highlighting; unused
/// languages are dropped from the build.
abstract final class HighlightClojure {
  /// The grammar for `clojure`.
...
final Grammar _g0 = Grammar([
  ...
  GrammarToken("string", compileHighlightPattern("\"(?:[^\"\\\\]|\\\\.)*\""),
      greedy: true),
  GrammarToken("char", compileHighlightPattern("\\\\\\w+")),
  GrammarToken("symbol",
      compileHighlightPattern("(^|[\\s()\\[\\]{},])::?[\\w*+!?'<>=/.-]+"),
      lookbehind: true),
  GrammarToken(
      "keyword",
