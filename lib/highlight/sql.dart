...
/// Syntax grammar for `sql`.
///
/// Import this library only when you need `sql` highlighting; unused
/// languages are dropped from the build.
abstract final class HighlightSql {
  /// The grammar for `sql`.
...
final Grammar _g0 = Grammar([
  ...
  GrammarToken("variable",
      compileHighlightPattern("@([\"'`])(?:\\\\[\\s\\S]|(?!\\1)[^\\\\])+\\1"),
      greedy: true),
  ...
  GrammarToken("identifier",
      ...
      lookbehind: true, greedy: true, inside: () => _g1),
  GrammarToken(
      "function",
      compileHighlightPattern(
  ...
  GrammarToken(
      "boolean",
      compileHighlightPattern("\\b(?:FALSE|NULL|TRUE)\\b",
          caseSensitive: false)),
  GrammarToken(
      "number",
      compileHighlightPattern(
