...
/// Syntax grammar for `apacheconf`.
///
/// Import this library only when you need `apacheconf` highlighting; unused
/// languages are dropped from the build.
abstract final class HighlightApacheconf {
  /// The grammar for `apacheconf`.
...
final Grammar _g0 = Grammar([
  ...
  GrammarToken(
      ...
      inside: () => _g1),
  GrammarToken(
      "directive-flags", compileHighlightPattern("\\[(?:[\\w=],?)+\\]"),
      alias: "keyword"),
  GrammarToken("string", compileHighlightPattern("(\"|').*\\1"),
      inside: () => _g5),
  GrammarToken(
      "variable", compileHighlightPattern("[\$%]\\{?(?:\\w\\.?[-+:]?)+\\}?")),
  GrammarToken("regex", compileHighlightPattern("\\^?.*\\\$|\\^.*\\\$?")),
]);

final Grammar _g1 = Grammar([
  GrammarToken("directive-block", compileHighlightPattern("^<\\/?\\w+"),
      alias: "tag", inside: () => _g2),
  GrammarToken("directive-block-parameter", compileHighlightPattern(".*[^>]"),
      alias: "attr-value", inside: () => _g3),
